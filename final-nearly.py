# -*- coding: utf-8 -*-
"""
Created on Tue Aug 12 21:18:35 2025
...
# -*- coding: utf-8 -*-
"""
Audio-Visual Synthesizer - Windows Compatible Version
----------------------------------------------------
- Uses pygame for audio instead of sounddevice (more stable on Windows)
- Simplified audio processing to avoid access violations
- Same hand tracking and gesture controls
- Fallback to visualization-only mode if audio fails
"""
        ...
        try:
            ...
            while audio_running.is_set():
                if not pygame.mixer.get_busy():
                    ...
                    chunk = self.get_audio_chunk(chunk_size)
                    
                    # Apply effects
                    with effects_lock:
                        effects_copy = current_effects.copy()
                    
                    processed_chunk = self.effects.process_audio(chunk, effects_copy)
...
def main():
    ...
    print("=== Audio-Visual Synthesizer Starting ===")
    ...
    try:
        ...
        if AUDIO_AVAILABLE:
            audio_running.set()
            audio_thread = threading.Thread(target=audio_manager.start_pygame_audio, daemon=True)
            audio_thread.start()
            print("Audio started")
        else:
            ...
            while cap.isOpened():
                ...
                if results.multi_hand_landmarks:
                    ...
                    if control_idx != -1:
                        ...
                        # Apply effects based on finger combinations
                        with effects_lock:
                ...
                cv2.imshow("Audio-Visual Synthesizer", image)
